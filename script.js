...
function applySettings() {
  ...
  // Amélioration — position du widget
  applyPosition(fieldData.widgetPosition || 'center');

  // Amélioration — preset de thème
  applyThemePreset(fieldData.themePreset || 'custom');
}

// ─── FIX #1 — hexToRgba robuste (supporte hex court #rgb et couleurs invalides)
...
// ─── Particules — FIX #5 nettoyage garanti en début d'alerte ────────────────
...
// ─── Amélioration — sons par type d'alerte ───────────────────────────────────
function playSound(type) {
  const soundUrl = fieldData['sound' + type.charAt(0).toUpperCase() + type.slice(1)];
...
// ─── Amélioration — avatar Twitch via decapi.me ──────────────────────────────
function loadAvatar(username) {
  if (!avatarEl) return;
  if (!fieldData.showAvatar) {
...
// ─── FIX #2 — File d'attente des alertes ────────────────────────────────────
...
window.addEventListener('onEventReceived', function (obj) {
  ...
  // Amélioration — support Bits/Cheer
